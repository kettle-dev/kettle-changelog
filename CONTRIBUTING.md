...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/kettle/changelog/my_class.rb` -> `spec/kettle/changelog/my_class_spec.rb`.
...
[🧪build]: https://github.com/kettle-dev/kettle-changelog/actions
