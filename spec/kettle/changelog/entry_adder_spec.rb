...
RSpec.describe Kettle::Changelog::EntryAdder do
  around do |example|
  ...
  it "fails when the changelog entry is empty" do
    expect { described_class.new(root: @root, section: "Changed", entry: "  ") }
      .to raise_error(Kettle::Changelog::Error, /changelog entry must not be empty/)
  end
  ...
  it "fails when the requested section is unsupported" do
    ...
    expect { adder.run }.to raise_error(Kettle::Changelog::Error, /unsupported changelog section "Invalid"/)
  end

  it "fails when the changelog is missing" do
    adder = described_class.new(root: @root, section: "Changed", entry: "Entry.")

    expect { adder.run }.to raise_error(Kettle::Changelog::Error, /missing CHANGELOG\.md/)
  end
  ...
  it "fails when there is not exactly one Unreleased section" do
    ...
    expect { adder.run }.to raise_error(Kettle::Changelog::Error, /expected exactly one ## \[Unreleased\] section/)
  end
  ...
  it "fails when the requested section is missing" do
    ...
    expect { adder.run }.to raise_error(Kettle::Changelog::Error, /expected at least one ### Changed section/)
  end
  ...
  it "wraps the Markly CRISPR load error in a kettle-dev error" do
    ...
    expect { adder.send(:require_markly_crispr!) }
      .to raise_error(Kettle::Changelog::Error, /requires ast-crispr-markdown-markly/)
  end
